...
async function detail(link) {
  ...
  $('.anim-senct .right-senc .spe span').each((_, e) => {
    ...
    if (t.includes(':')) {
      ...
      // Fix: pakai rest.join(':') agar nilai yang mengandung ':' tidak terpotong
      info[k.trim().toLowerCase().replace(/\s+/g, '_')] = rest.join(':').trim();
    }
  });
...
app.get('/api/watch', async (req, res) => {
  try {
    if (!req.query.url) return res.status(400).json({ error: 'url is required' });
    const data = await getWatch(req.query.url);
    res.json(data);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// ── SERVER ───────────────────────────────────────────────────────
// Kompatibel dengan: Node.js lokal (listen) & Vercel serverless (module.exports)

if (require.main === module) {
  // Jalan langsung: node api/index.js
